 ...
 * 唱片物品。
 *
 * <p>1.21 起唱片改为数据驱动：{@code RecordItem} 已被移除，普通物品只要带上
 * {@code jukebox_playable} 数据组件（指向 {@link JukeboxSong} 注册表中的一个条目）
 * 即可放入唱片机。因此这里继承 {@link Item}，在构造时把歌曲 key 写进物品属性。
 * 歌曲本身（声音、时长、比较器输出）由数据包里的 {@code data/<ns>/jukebox_song/<id>.json} 定义。
 */
public class FlexRecordItem extends Item implements IEventRunner {
    public FlexRecordItem(ResourceKey<JukeboxSong> song, Properties properties, ItemBuilder builder) {
        super(properties.stacksTo(1).jukeboxPlayable(song));
