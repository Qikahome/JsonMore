...
public class FlexStandingAndWallBlockItem extends StandingAndWallBlockItem
    ...
    public InteractionResultHolder<ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
        ...
        if (useTime != null && useTime > 0)
            ...
            return runEvent(FlexEventType.USE_ITEM_ON_AIR, FlexEventContext.of(worldIn, playerIn, handIn, heldItem),
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        var result = runEvent(FlexEventType.USE_ITEM_ON_BLOCK, FlexEventContext.of(context),
                () -> new InteractionResultHolder<>(super.useOn(context), heldItem));
    ...
    public ItemStack finishUsingItem(ItemStack heldItem, Level worldIn, LivingEntity entityLiving) {
        var result = runEvent(FlexEventType.END_USING, FlexEventContext.of(worldIn, entityLiving, heldItem),
                () -> InteractionResultHolder.success(super.finishUsingItem(heldItem, worldIn, entityLiving)));
        if (result.getResult() != InteractionResult.SUCCESS)
            return result.getObject();

        return runEvent(FlexEventType.USE_ITEM_ON_AIR, FlexEventContext.of(worldIn, entityLiving, heldItem),
                () -> result).getObject();
    }
