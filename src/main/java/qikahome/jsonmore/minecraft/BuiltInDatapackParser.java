    ...
    public static class Builder extends BaseBuilder<RepositorySource, Builder> {
        ...
        protected RepositorySource buildInternal() {
            ...
            if (modContainer.isEmpty()) {
                ...
                // Fabric 无 AddPackFindersEvent，走 Fabric 内置包机制。该机制固定按
                // resourcepacks/<path> 取包目录（公开 API 改不了 subPath），故 Fabric 版的数据包
                // 目录约定为 resourcepacks/<path>/（见 docs，与 Neo 的 datapacks/ 不同）。
